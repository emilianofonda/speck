...
def ecscanActor(fileName,e1,e2,n=1,dt=0.04,velocity=10, e0=-1, mode="",shutter=False,beamCheck=True):
    ...
    #Finalize derived quantities
                fluoX = zeros(cardXiaDataShape[0],"f")
                #numpy.nan_to_num(array( sum(mcaSum[:,roiStart:roiEnd], axis=1), "f") / I0)
                prx = "outtaHDF.root.XIA."
                for Chan in range(nChan):
                    fluoX += eval(prx + "fluo%02i"%Chan)[:]*(100./(100.-eval(prx + "deadtime%02i"%Chan)[:]))
                fluoX = fluoX/I0
    ...
    #Local data saving
                ...
                I0,I1,I2,I3],"f")
                header = ""
                header += "#.txt file columns content is:\n"
                header += "#1) Energy\n"
                header += "#2) Angle\n"
                header += "#3) Transmission\n"
                header += "#4) Fluorescence\n"
                header += "#5) Standard\n"
                header += "#6) I0\n"
                header += "#7) I1\n"
                header += "#8) I2\n"
                header += "#9) I3\n"
                header += "#TimeAtStart = %s\n" % (timeAtStart))
                header += "#TimeAtStop  = %s\n" % (timeAtStop))
                header += "#Scan from %g to %g at velocity= %g eV/s\n" % (e1, e2, velocity))
                header += "#Counter Card Config\n")
                for i in cardCTsavedAttributes:
                header += "#%s = %g\n" % (i,cardCT.read_attribute(i).value)
                header += "#Analog  Card Config\n"
                #Report  dark currents applied
                header += "#Dark_I0= %9.8f\nDark_I1= %9.8f\nDark_I2= %9.8f\nDark_I3= %9.8f\n"\
                %(cardAI_dark0,cardAI_dark1,cardAI_dark2,cardAI_dark3,)
                #
                ...
                numpy.savetxt(ActualFileNameData, transpose(dataBlock), header = header)
