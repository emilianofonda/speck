...
#            print "Cannot read mirror 2"
    ...
    try:
        __encoders_output_file=open(fname,"w")
    except:
...
def SetPositions(filename=""):
    ...
    if(filename==""):
        if not(NOTK):
            filename=tkinter.filedialog.askopenfilename(initialdir=__pySamba_root+"/files/encoders/",title="Choose the CORRECT getposition file")
    ...
    answer=input("Do you REALLY want to apply these values now? [Yes/]")
    if(answer=="Yes"):
        failures=0
        failedmotors=[]
        for i in NewMotPos:
