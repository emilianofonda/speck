...
class motor_slit:
        """Alternative to the slit class: define one of the four pseudo-motors per each slit: pos,gap,Up/In,Down/Out. Each has standard motor attributes and special behaviour with regard to speed and acceleration and so on. You must provide the device address of the slits, of the two motors and the keyword corresponding to the attribute you want to control: pos, gap, up (or) in,down (or) out.
