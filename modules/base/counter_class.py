...
class counter(object):
    ...
    def __repr__(self):
        ...
        if len(tmp)<=7:    
            ...
            s+="\n"
            for j in range(0,old_div((len(tmp)-7),8)):    
    ...
    def read(self):
        ...
        try:
            values = [x.value for x in self.DP.read_attributes(self.counter_names)]
    ...
    def readRawData(self):
        ...
        try:
            return [x.value for x in self.DP.read_attributes(self.counter_names)]
        except PyTango.DevFailed as tmp:
            print("Cannot read counters... ", end=' ')
            raise tmp
        except PyTango.CommunicationFailed as tmp:
    ...
    def writeDark(self):
        ...
        dark = list(old_div(array(self.readRawData(),"f") * array(self.mask), self.DP.integrationTime))
        ...
        self.DP.put_property({'SPECK_DARK_VALUES': list(map(str, dark))})
