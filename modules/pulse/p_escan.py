...
class escan_class:
    ...
    Ts2_Moves=False,amplis=None):
        ...
        #Force pseudo counter configuration update
        self.cpt.reinit()
        col=0
        ...
        while (iscan<nscans-1):
            ...
            while(pointIndex < len(self.trajectory["energy"])):
                try:    
                    ...
                    shortLineBuffer += ("%-8.2f\t%8.6f\t%8.6f\t%8.6f\t%8.6f\t" % (etheta, theta, mu, fluopoint[0], mus))
                    try:
                        individualMUF = ["%9.7f"%cnts[i]/i0 for i in self.auto_fluo_channels]
                    except:
                        individualMUF = ["%9.7f"%0 for i in self.auto_fluo_channels]
                    #shortLineBuffer += "\t".join(individualMUF)
...
#            if self.TUNING:
        ...
        handlerFull.writelines(buffer)
        return buffer
        
    def pre_recording(self,handler=None,handlerFull=None,nowait=False):
        if(not(nowait)):
            try:
