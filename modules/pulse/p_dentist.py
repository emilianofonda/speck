...
def load_xas_from_hdf(filename):
    """Load the file from HDF and returns m that stores data like it was in txt file of older version of ecscan.
    It depends on a naming convention in the HDF file:
    energy,I0,I1,I2, FLUO,FLUO_RAW,MUX are found in posts
    """
    #    This function may be used as template for another that converts hdf to txt in that case more columns have to be exported.   
...
mode="t", figN=1, overlap=False,out=False):
    ...
    bkgPoly = scipy.optimize.curve_fit(polySc,chiK, chi, [1,]*int(polyN), sigma=1./chiK**kweight)[0]
    ...
    pylab.subplot(221)
    if not overlap:
        pylab.title(filename[filename.rfind(os.sep):filename.rfind(".")])
    pylab.grid()
    if not overlap:
