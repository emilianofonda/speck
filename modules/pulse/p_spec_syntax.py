...
#Now move file from temporary folder to ruche
        IPy = get_ipython()
        if self.final_filename.startswith(IPy.user_ns["__SPECK_CONFIG"]["SOLEIL_DATA_ROOT"]):
            shutil.move(self.handler.filename, self.final_filename)
        elif self.final_filename.startswith(IPy.user_ns["__SPECK_CONFIG"]["USER_DATA_ROOT"]):
