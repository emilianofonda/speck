...
#           raise Exception("%s: attempt to read scalers data when not in ON state"%self.DP.name)
    ...
    def prepareHDF(self, handler, HDFfilters = tables.Filters(complevel = 1, complib='zlib'),upperIndex=()):
        ...
        ShapeMatrices = (self.DP.acq_nb_frames, self.DP.image_width - 10)+ tuple(self.upperDimensions)
...
#Write down contextual data
        ...
        handler.create_carray(outGroup, "real_pixels_list", title = "List of Real Pixels: 13,17,19,21",\
        shape = (1,), atom = tables.StringAtom(256,1))
            ...
            try:
                ...
                for i in range(self.numChan):
                    ...
                    if upperIndex == ():
                        ...
                        outNode[p0:p1][upperIndex] = sourceFile.root.entry_0000.measurement.xspress3.data[::reverse,i,0:nBins]
