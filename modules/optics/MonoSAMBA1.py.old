#Monochromator1 of SAMBA: Sagittaly focusing double crystal monochromator.
#This version of the code is extremely simplified and uses the TANGO device (iiop version 1.2).
#The bender is managed by TANGO too. Use the mono1b.py version for a higher speed version of the code.
#The mono1b.py version directly ùmanages themotors at lower level.
...
class mono1:
	...
	tuning_counter,tuning_channel=0,
	deadtime=0.05,timeout=0.,delay=0.,
	modes={"PseudoChannelCut":1,"TunedChannelCut":2,"FixedExit1":3,"FixedExit2":4}
	):
		self.DP=DeviceProxy(label)
		self.label=label
		try:
			self.counter=counter(tuning_counter)
		except:
			self.counter=None
			print "No tuning will be possible: counter error!"
		self.counter_channel=tuning_channel
		self.timeout=timeout
		self.deadtime=deadtime
		self.delay=delay
		self.modes=mode
	...
	def __repr__(self):
		...
		if self.state() in [DevState.FAULT,DevState.ALARM]: 
			...
			color=""
		return self.label+"/energy  at %8.2f"%self.pos()+" eV is in state: "+color+"%s"%(self.state())+RESET

	def subtype(self):
		return "SAMBA DOUBLE CRYSTAL MONOCHROMATOR"
	
	def mode(mode=None):
		if mode==None:
		    mode=self.DP.currentFunctioningMode
		    return self.modes[mode]
		elif mode>len(self.modes.keys()) or mode<1:
		    return self.modes_rev[mode]
		else:
		    self.DP.ChooseFunctioningMode(mode)
		    sleep(self.deadtime)
		return self.mode()
	...
	def usebender(self,value=None):
		"""Now this function must modify a property (since we miss an attribute)
		and """
	...
	def sample_at(self,distance=None):
		if distance==None:
			return self.DP.qDistance
		else:
			if distance>17.5:
			    try:
				self.DP.qDistance=distance
			    except:
				pass
			else
			    self.DP.qDistance=distance
